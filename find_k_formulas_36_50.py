#!/usr/bin/env python3
"""Find k-value formulas for n=36-50."""
import sqlite3
...
def find_formula(n, K, max_coeff=500, max_const=10000):
    results = []
    for i, j in combinations(range(max(1, n-25), n), 2):
        ...
        for a in range(-max_coeff, max_coeff+1):
            ...
            for b in range(-max_coeff, max_coeff+1):
                ...
                c = remainder - b * K[j]
                if abs(c) <= max_const:
                    score = abs(a) + abs(b) + abs(c)//100
                    results.append((score, a, i, b, j, c))
    results.sort()
    return results[:1] if results else []

print("VERIFIED K-FORMULAS (n=36-50)")
print("=" * 70)
